...
test('pageShell keeps the accessible theme toggle button in both script modes', () => {
  const plain = shell();
  const hosted = shell({ siteScripts: true });

  for (const html of [plain, hosted]) {
    assert.match(html, /<button class="theme-toggle" id="theme-toggle"/);
    assert.match(html, /aria-label="다크 모드로 전환"/);
  }
});
