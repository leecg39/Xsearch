...
test('topicFromTitle reads the newsletter suffix and falls back to AI', () => {
  ...
  assert.equal(topicFromTitle('본 제목 | 오늘의 경제/금융 브리핑'), 'finance');
  assert.equal(topicFromTitle('본 제목 | 오늘의 개발/테크 브리핑'), 'dev');
  assert.equal(topicFromTitle('본 제목 | 오늘의 스타트업/비즈 브리핑'), 'startup');
...
test('topicFromReport prefers report.topic and ignores blanks', () => {
  ...
  assert.equal(topicFromReport({ topic: 'nope' }, '오늘의 경제/금융 브리핑'), 'ai');
  assert.equal(topicFromReport({ topic: '' }, '오늘의 개발/테크 브리핑'), 'dev');
  assert.equal(topicFromReport({ topic: '   ' }, '오늘의 스타트업/비즈 브리핑'), 'startup');
  assert.equal(topicFromReport(null, '오늘의 경제/금융 브리핑'), 'finance');
...
test('siteBrand uses a generic name when topics are mixed', () => {
  ...
  assert.equal(siteBrand([{ topic: 'finance' }]).newsletter, '오늘의 경제/금융 브리핑');
  assert.equal(siteBrand([{ topic: 'finance' }, { topic: 'finance' }]).newsletter, '오늘의 경제/금융 브리핑');
...
test('buildArchive brands a finance report and infers topic from digest titles', async () => {
  ...
  try {
    ...
    await writeFile(
      ...
      '<!doctype html><title>X 트렌드 다이제스트 — 2026-08-17 | 오늘의 개발/테크 브리핑</title><main>다이제스트 본문</main>',
      'utf8',
    );
    ...
    assert.match(html, /오늘의<br>경제\/금융 브리핑/);
    assert.match(html, /경제\/금융 브리핑/);

    const digestCard = html.match(/<a class="card" href="2026-08-17\.html"[\s\S]*?<\/a>/)?.[0] ?? '';
    assert.match(digestCard, /오늘의 개발\/테크 브리핑/);
